...
class BarCodeScannerViewController: UIViewController , AVCapturePhotoCaptureDelegate {
    ...
    private func showInfo(for payload: String) {
        ...
        if let product = productCatalog.item(forKey: payload) {
            ...
            showAlert(withTitle: product.name ?? "Bar code scanned success !", message: payload)
            self.delegate?.sendProductData(scannedData: product)
        } else {
